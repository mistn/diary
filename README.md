...
- 阅读：`/` 日记全文流（20 篇一页，`?before=` 翻更早），`/w/:id` 详情，纯 HTML+CSS，无前端 JS（`/d` 跳 `/`，仅兼容老链接）
- 写作：`/admin` 写 Markdown，标题可空（不填只显示日期），支持编辑/删除，存 `D1`
- 可见：公开勾掉即私密，列表/详情/RSS 全不可见，登录后自己可见
- 图片：`/admin` 选图上传 `R2`，自动压 webp（长边 1600）并插到光标处，前台 `loading=lazy` 懒加载
...
首次建站：`npx wrangler d1 create weekly`（id 填进 `wrangler.toml`）+ `npx wrangler r2 bucket create diary-img`，表由代码 `ensure()` 自动建。
